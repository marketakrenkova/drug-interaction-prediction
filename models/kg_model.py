...
"""
Knowledge Graph Model Training Script

This script trains and evaluates a knowledge graph (KG) model using the PyKEEN library. It supports loading, training, and predicting with KG models, and provides flexibility for specifying model parameters through command-line arguments.

In "../data/triplets/{run}/{data_dir}", there must be files with triplets (KG) that are used for the training:
    - train_{data_name}.tsv
    - valid_{data_name}.tsv
    - test_{data_name}.tsv,

Usage:
    ./kg_model.py -d <data_name> -m <model> -s <model_specification> ...

Command-line Arguments:
    -d, --data_name: Name of the dataset.
    -m, --model: Type of the knowledge graph model (default: "TransE").
    -s, --model_specification: Specification string for the model.
    -emb, --embedding_dim: Dimensionality of entity embeddings (default: 32).
    -e, --epochs: Number of training epochs (default: 1).
    -o, --optimizer: Optimizer for training (default: "Adam").
    -lr, --learning_rate: Learning rate for the optimizer (default: 0.01).
    -l, --loss: Loss function for training (default: "MarginRankingLoss").
    -b, --batch_size: Batch size for training (default: 64).
    -lm, --loss_margin: Margin for the loss function (default: 1.0).
    -neg, --neg_sampler: Negative sampler for training (default: "basic").
    -nn, --num_neg_per_pos: Number of negative samples per positive sample (default: 1).
    --not_train: Flag to skip training and only predict with a pre-trained model.
    -r, --run: Run identifier (default: "run1").

Example:
    ./kg_model.py -d sample_data -m TransE -s baseline -emb 64 -e 10 -o Adam -lr 0.001
...
class DataLoader:
    ...
    def load(self):
        train_df = pd.read_csv(
            self.data_dir + "train_" + self.data_name + ".tsv", sep="\t", engine="python",
        )  
        valid_df = pd.read_csv(
            self.data_dir + "valid_" + self.data_name + ".tsv", sep="\t", engine="python",
        )
        test_df = pd.read_csv(
            self.data_dir + "test_" + self.data_name + ".tsv", sep="\t", engine="python"
        )
...
class KG_model:
    ...
    def __str__(self):
        print(
            f"Training {self.model_name} - {self.specification}-{self.data_name} for {self.num_epochs} epochs on {self.device}."
        )
    ...
    def train(self, model_checkpoint_path):
        self.trained_model = pipeline(
            ...
            loss=self.loss,
            loss_kwargs=dict(margin=self.margin),
            optimizer=self.optimizer,
            optimizer_kwargs=dict(lr=self.learning_rate),
            negative_sampler=self.negative_sampler,
            negative_sampler_kwargs=dict(num_negs_per_pos=self.num_neg_per_pos),
            evaluator=None,
            device=self.device,
    ...
    def predict_tail(self, trained_model, triples, head, relation, filter_known=False):
        prediction_dir = (
            "../predictions/" + self.specification + "/" + self.data_name + "/"
        )
        ...
        try:
            ...
            if filter_known:
                ...
                pred = pred_filtered.add_membership_columns(
                    validation=self.valid_tf, testing=self.test_tf
                )

            predicted_tails_df = pred.df.head(100)
            predicted_tails_df.to_csv(
                prediction_dir + self.model_name + "_" + head + "_" + relation + "_" + self.specification + ".csv"
            )
    ...
    def save_metrices(self):
        ...
        print(df_result)
        df_result.to_csv(
            prediction_dir + self.specification + self.data_name + "_metrices.csv"
        )
...
def main(args):
    ...
    model_checkpoint_path = (
        f"{args.model}-{model_specification}-{args.data_name}_checkpoint.pt"
    )
    model_result_dir = (
        f"results/results-{args.model}_{model_specification}-{args.data_name}"
    )
    ...
    if PREDICT:
        ...
        if args.not_train:
            ...
            loaded_model = load_model(
                "kg_checkpoints/" + model_checkpoint_path,
                model_result_dir + "/trained_model.pkl",
            )
        ...
        for d in common_drugs[:100]:
            if loaded_model is None:
                kg.predict_tail(
                    kg.trained_model.model,
                    kg.trained_model.training,
                    d,
                    "interacts",
                    filter_known=True,
                )
                ...
                kg.predict_tail(
                    loaded_model, data.train, d, "interacts", filter_known=True
                )
        ...
        for food in foods:
            if loaded_model is None:
                ...
                kg.predict_tail(
                    loaded_model, data.train, food, "interacts", filter_known=True
                )

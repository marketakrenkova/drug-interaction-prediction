...
"""
Simple drug-food predictor 

Predicts possible drug-food interactions according to common compounds.
...
Creates a pandas.DataFrame containing drug, type of interaction, food and the common compound
"""
...
class SimpleDFIPredictor:
    ...
    def load_data(self):
        ...
        self.drug_ingredients = pd.read_csv(
            self.drug_ingredients_path, sep="\t", index_col=[0]
        )
        self.food_ingredients = pd.read_csv(
            self.food_ingredients_path, sep="\t", index_col=[0]
        )
    ...
    def create_food_compound_dict(self):
        ...
        for c in compounds:
            food_compound_dict[c] = list(
                self.food_ingredients[self.food_ingredients["compound_id"] == c].food_id
            )
    ...
    def create_id2name_mapping(self):
        ...
        self.compound_id_map_dict = dict(
            zip(self.compounds["name"], self.compounds["public_id"])
        )
        self.drug_id_map_dict = dict(
            zip(self.drug_id_map.id, self.drug_id_map.drug_name)
        )
        self.food_id_map_dict = dict(
            zip(self.food_id_name.public_id, self.food_id_name.name)
        )
    ...
    def create_drug_compound_dict(self):
        ...
        interacting_drugs.union(
            set(self.ddi.drug2)
        )  # doesn't have to be drug, it can be a compound too

        for d in interacting_drugs:
            drug_compound_dict[d] = list(
                self.drug_ingredients[
                    self.drug_ingredients["ingredient"] == d
                ].drug_name
            )
    ...
    def predict_dfi(self):
        ...
        for row in self.ddi.itertuples():
            ...
            if drug2_ingredients is not None:
                ...
                if (
                    len(sim_food) > 0 and len(sim_food) <= 20
                ):  # if sim food > 20 -> probably no interaction
                    for compound, food in sim_food.items():
                        for f in food:
                            # possible_interactions.append([drug1, effect, f, compound])
                            possible_interactions.append(
                                [
                                    self.drug_id_map_dict[drug1],
                                    effect,
                                    self.food_id_map_dict[f],
                                    compound,
                                ]
                            )
        ...
        self.possible_dfi = pd.DataFrame(
            possible_interactions,
            columns=["drug", "interaction", "food", "common_compound"],
        )
...
if __name__ == "__main__":
    ...
    dfi_predictor.save("../results/possible_dfi.csv")

...
"""
OGB Link Prediction Script

This script performs link prediction on an Open Graph Benchmark (OGB) dataset using PyTorch Geometric and PyKEEN. It loads a specified OGB dataset, transforms it, trains a knowledge graph (KG) model, and evaluates the model's performance.
...
def load_data(dataset_name):
    dataset = PygLinkPropPredDataset(
        name=dataset_name, root="../data/dataset-ogb/", transform=T.ToSparseTensor()
    )
...
def save_to_txt(dir_data_my_split, train_df, valid_df, test_df):
    train_df.to_csv(
        dir_data_my_split + "train.txt", sep="\t", header=False, index=False
    )
    valid_df.to_csv(
        dir_data_my_split + "valid.txt", sep="\t", header=False, index=False
    )
...
def compute_scores(trained_model, train, valid, valid_neg, test, test_neg):
    ...
    for i in range(n + 1):
        ...
        pos_train_preds += [
            trained_model.model.score_hrt(edge).squeeze().cpu().detach()
        ]
...
def main():
    print("Loading data...")
    dataset_name = "ogbl-biokg"  # ogbl-ddi
    ...
    config = {
        "metadata": dict(title=model_name),
        "pipeline": dict(
            ...
            stopper="early",
            stopper_kwargs=dict(patience=5, relative_delta=0.002),
    ...
    if "ddi" in dataset_name:
        ...
        (
            ...
            pos_test_pred,
            neg_test_pred,
        ) = compute_scores(pipeline_result, train, valid, valid_neg, test, test_neg)
        evaluate_ogb(
            pos_train_pred,
            pos_valid_pred,
            neg_valid_pred,
